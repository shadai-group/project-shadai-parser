...
class LLMClient:
    ...
    def _create_provider(
        ...
        if provider == "gemini":
            ...
            raise ValueError(
                f"Unsupported provider: {provider}. Supported providers: gemini, anthropic, openai"
            )
    ...
    def _get_api_key_from_env(self, provider: str) -> str:
        ...
        if not api_key:
            raise ValueError(
                f"API key not found. Please set {env_var} environment variable or pass api_key parameter."
            )
    ...
    def generate_with_documents(self, prompt: str, documents, **kwargs) -> str:
        ...
        response = self.llm_provider.generate_with_documents(
    ...
    def parse_pdf(
        ...
        """Parse PDF with LLM."""
        response = self.pdf_parser.parse_with_llm(
            pdf_path, prompt, use_images, **kwargs
        )
        return response.content
